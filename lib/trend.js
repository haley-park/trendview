...
export const CATEGORY_QUERIES = {
  ...
  china: { ko: "중국", en: "china travel", zh: "中国", ja: "中国", es: "china viaje" },
  expat: { ko: "해외생활", en: "expat life", zh: "海外生活", ja: "海外生活 移住", es: "vivir en el extranjero" },
...
// 인스타 임베드 페이지(로그인 불필요)를 Jina로 렌더링해 최근 게시물을 추출.
// 임베드 그리드에는 조회수/좋아요가 없어 썸네일·링크·캡션·시각만 제공된다.
function parseIgEmbed(html, username) {
  const blocks = html.split('\\"shortcode_media\\"').slice(1);
  const items = [];
  for (const block of blocks) {
    const code = block.match(/\\"shortcode\\":\\"([A-Za-z0-9_-]+)\\"/)?.[1];
    if (!code) continue;
    const isVideo = /\\"is_video\\":true/.test(block);
    let thumb = block.match(/\\"display_url\\":\\"(https:[^"]+?)\\"/)?.[1] || "";
    thumb = thumb.replace(/\\+\//g, "/").replace(/\\u0026/g, "&");
    const takenAt = parseInt(block.match(/\\"taken_at_timestamp\\":(\d+)/)?.[1] || "0", 10);
    let title = "";
    const cap = block.match(/\\"edge_media_to_caption\\"[\s\S]{0,200}?\\"text\\":\\"([\s\S]*?)\\"\}/)?.[1];
    if (cap) {
      try {
        title = JSON.parse('"' + cap.replace(/\\\\/g, "\\") + '"').split("\n")[0].slice(0, 120);
      } catch {
        title = cap.replace(/\\+n/g, " ").replace(/\\+/g, "").split("\n")[0].slice(0, 120);
      }
    }
    items.push({
      account: username,
  ...
  const videos = unique.filter((it) => it.isVideo);
  return (videos.length ? videos.concat(unique.filter((it) => !it.isVideo)) : unique).slice(0, 12);
}

async function fetchIgReelsViaJina(username) {
  await new Promise((r) => setTimeout(r, Math.floor(Math.random() * 800)));
  for (let i = 0; i < 2; i++) {
    try {
      const html = await jinaText(
        "https://www.instagram.com/" + encodeURIComponent(username) + "/embed/", "html", 15000);
      const items = parseIgEmbed(html, username);
...
export async function getReels(accounts, force) {
  const fetchFn = async () => {
    ...
    let reels = results.flatMap((r) => r.reels);
    if (!reels.length) {
      // 데이터센터 IP 차단 시 임베드 페이지를 Jina로 렌더링해 실제 콘텐츠 확보
      const viaJina = await mapPool(accounts.slice(0, 6), 2, (a) => fetchIgReelsViaJina(a));
      reels = viaJina.flat();
      reels.sort((a, b) => (b.takenAt || 0) - (a.takenAt || 0));
      const accountStats = accounts.map((a) => ({ account: a, followers: 0, avgEngagement: 0, rate: 0 }));
...
async function fetchXPostsViaJina(username) {
  const url =
    "https://syndication.twitter.com/srv/timeline-profile/screen-name/" +
    encodeURIComponent(username);
  // jina 무료 티어(분당 20회) 제한에 걸리면 잠시 후 1회 재시도. 버스트 방지용 지터 포함
  await new Promise((r) => setTimeout(r, Math.floor(Math.random() * 800)));
  for (let i = 0; i < 2; i++) {
    try {
      const html = await jinaText(url, "html", 15000);
      const posts = parseXHtml(html, username);
      if (posts.length) return posts;
    } catch {}
    if (i === 0) await new Promise((r) => setTimeout(r, 3000));
...
export async function getXPosts(accounts, force) {
  const fetchFn = async () => {
    ...
    if (posts.length) return posts;
    // 2차: 데이터센터 IP 차단 시 Jina 리더 경유 — 분당 한도 회피를 위해 동시성 2
    const subset = accounts.slice(0, 6);
    const viaJina = await mapPool(subset, 2, (a) => fetchXPostsViaJina(a));
    return viaJina.flat();
  };
...
async function fetchThreadsViaJina(username) {
  ...
  for (let i = 0; i < 2; i++) {
    try {
      const md = await jinaText("https://www.threads.com/@" + encodeURIComponent(username), "markdown", 15000);
      const posts = parseThreadsMarkdown(md, username);
      if (posts.length) return posts;
    } catch {}
    if (i === 0) await new Promise((r) => setTimeout(r, 3000));
  }
...
export async function getThreadsPosts(accounts, force) {
  const fetchFn = async () => {
    ...
    let posts = results.flat();
    if (posts.length) return posts;
    // 2차: Jina 리더가 렌더링한 프로필 페이지 파싱 (분당 한도 회피 — 동시성 2)
    const viaJina = await mapPool(accounts.slice(0, 5), 2, (a) => fetchThreadsViaJina(a));
    return viaJina.flat();
  };
...
async function fetchTiktokSearch(keywords, region) {
  const d = await tikwmJSON(
    `${TIKWM_BASE}/feed/search?keywords=${encodeURIComponent(keywords)}&count=30&region=${region}`);
  return (d?.data?.videos || []).map(tiktokItem);
}

// 트렌딩/카테고리 파트만 — 첫 화면을 빠르게 띄우기 위한 경량 조회 (tikwm 1회)
export async function getTiktokTrendingPart(region, category, lang, force) {
  const fetchFn = async () => {
    if (!category || category === "all") return fetchTiktokTrending(region);
    const cq = CATEGORY_QUERIES[category];
    const q = cq ? cq[lang] || cq.ko : category;
    return fetchTiktokSearch(q, region);
  };
  return cached(`tiktok:tr:${region}:${category || "all"}:${lang}`, force, fetchFn);
}

// 구독 계정 영상 + 팔로워 통계 파트 — 트렌딩 렌더 후 백그라운드로 병합
export async function getTiktokAccountsPart(accounts, force) {
  const fetchFn = async () => {
    const chunks = await mapPool(accounts, 2, (a) => fetchTiktokUser(a));
    const posts = chunks.flat();
    ...
    const accountStats = accounts.map((a, i) => {
      const mine = posts.filter((p) => String(p.account || "").toLowerCase() === a.toLowerCase());
    ...
    return { posts, accountStats };
  };
  return cached(`tiktok:acc:${accounts.join(",")}`, force, fetchFn);
}
...
export async function getXhsHot(force) {
  const fetchFn = async () => {
    // 1순위: 60s API (Cloudflare Workers, 무료 공개) — 미러 포함
    for (const base of ["https://60s.viki.moe", "https://60s-api.viki.moe"]) {
      try {
        const d = await httpJSON(base + "/v2/rednote", { timeout: 10000 });
...
export { httpGet, httpJSON, httpText, UA, parseThreadsMarkdown, parseXHtml, parseIgEmbed };
