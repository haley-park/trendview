...
async function fetchXPostsViaJina(username) {
  const url =
    "https://syndication.twitter.com/srv/timeline-profile/screen-name/" +
    encodeURIComponent(username);
  // jina 무료 티어(분당 20회) 제한에 걸리면 잠시 후 1회 재시도
  for (let i = 0; i < 2; i++) {
    try {
      const html = await jinaText(url, "html", 20000);
      const posts = parseXHtml(html, username);
      if (posts.length) return posts;
    } catch {}
...
async function fetchThreadsViaJina(username) {
  for (let i = 0; i < 2; i++) {
    try {
      const md = await jinaText("https://www.threads.com/@" + encodeURIComponent(username), "markdown", 20000);
